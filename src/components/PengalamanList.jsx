...
const PengalamanList = () => {
  ...
  const fetchPengalamans = async () => {
    try {
      const response = await axios.get('https://api.tegararsyadani.my.id/api/public/pengalamans/');
      setPengalamans(response.data.data);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://api.tegararsyadani.my.id/api/admin/pengalaman/${id}`,
        {
          headers: {
      ...
      fetchPengalamans(); // Refresh list after deletion
    } catch (error) {
  ...
  const currentPengalamans = pengalamans.slice(indexOfFirstItem, indexOfLastItem);
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      >
        <div className="flex-1 p-6 transition-all duration-300 bg-white shadow-lg rounded-lg">
          <h2 className="text-3xl font-bold mb-6 text-gray-700">Daftar Pengalaman</h2>
          ...
          <Link to="/pengalaman/create" className="bg-blue-600 hover:bg-blue-700 text-white p-3 rounded inline-flex items-center mb-6">
            <FiPlus className="mr-2" /> Tambah Post Pengalaman
          </Link>
          ...
          <table className="min-w-full bg-white border-collapse border border-gray-200">
            ...
            <tbody>
              {currentPengalamans.map((pengalaman) => (
                <tr key={pengalaman.id} className="hover:bg-gray-100 transition-all">
                  <td className="border px-4 py-2">{pengalaman.title}</td>
                  <td className="border px-4 py-2 flex items-center space-x-2">
                    <Link to={`/pengalaman/edit/${pengalaman.id}`} className="text-blue-600 hover:text-blue-800 flex items-center">
