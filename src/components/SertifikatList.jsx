...
const PortfolioList = () => {
  ...
  const fetchPortfolios = async () => {
    try {
      const response = await axios.get('https://api.tegararsyadani.my.id/api/public/sertifikats/');
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://api.tegararsyadani.my.id/api/admin/sertifikat/${id}`,
        {
          headers: {
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      >
        <div className="flex-1 p-6 transition-all duration-300 bg-white shadow-lg rounded-lg">
          <h2 className="text-3xl font-bold mb-6 text-gray-700">Daftar Sertifikat</h2>
          ...
          <Link to="/sertifikat/create" className="bg-blue-600 hover:bg-blue-700 text-white p-3 rounded inline-flex items-center mb-6">
            <FiPlus className="mr-2" /> Tambah Post Sertifikat
          </Link>
